...
FS.RouteBoard = {
    ...
    MobileCRM.UI.IFrameForm.requestObject(
      function (iFrame) {
        MobileCRM.UI.IFrameForm.show(
          "Missed Reason",
          "file://LeaseInspection/RouteBoardMissed.html",
          false,
          (options = { brbid: brbRecordId })
        );
      },
  ...
  toggleMenu: function () {
    ...
    filterEvent.addEventListener("click", toggle, false);
    function toggle() {
      filterText.innerHTML == "Filter";
      if (filterText.innerHTML == "Filter" 
      || $("#filterText").text()== "Filter") {
        filterText.innerHTML = "CURRENT VIEW";
        $(".chevronDown").hide();
        $(".chevronUp").show();
        $(".filter-open").show();
      } else {
  ...
  buildTable: function (itemArray) {
    ...
    var numberRecords = "# of jobs: " + itemArray.length.toString();
    ...
    for (var i in itemArray) {
      ...
      brbRows =
        ...
        ";' id='" +
        currentRecord.PadId +
        "'  onclick='FS.RouteBoard.onbrbRowClick(this)'>" +
        "<td class='irocTitle'  colspan='4'><img style='height: 25px; display: " +
        showLateIcon +
        ";' src='late.svg'>" +
        displayName +
        "</td>" +
        ...
        currentRecord.Color +
        ";'>" +
        "<td class='irocBody irocRouteSeperator' style='width: 40%' id='" +
        currentRecord.PadId +
        "'  onclick='FS.RouteBoard.onbrbRowClick(this)'>" +
        currentRecord.ResourceName +
        "</td>" +
        "<td class='irocIcon irocRouteSeperator' style='width: 15%; max-height=40'><img id=" +
        currentRecord.BRBId +
        " style='height: 40px; visibility: " +
        showHideIcons +
        "' src='feed.svg' onclick='FS.RouteBoard.onCreateRouteFeedClick(this)'></td>" +
        "<td class='irocIcon irocRouteSeperator' style='width: 15%'><img id=" +
        currentRecord.BRBId +
        " style='height: 40px; visibility: " +
        showHideMissedIcon +
        "' src='missed.svg' onclick='FS.RouteBoard.onReasonBtnClick(this)'></td>" +
        "<td class='irocIcon irocRouteSeperator' style='width: 15%'><img id=" +
        currentRecord.BRBId +
        " style='height: 40px; visibility: " +
        showHideIcons +
        "' src='reassign.svg'  onclick='FS.RouteBoard.onAssignBtnClick(this)'></td>";

      if (currentRecord.LeaseRequired) {
        brbRows =
          brbRows +
          "<td class='irocIcon irocRouteSeperator' style='width: 15%' ><img id=" +
          currentRecord.LeaseLink +
          " style='height: 40px'  src='lease2.svg' onclick='FS.RouteBoard.onLeaseClick(this)'</td></tr>";
      } else {
        brbRows =
          brbRows +
          "<td class='irocIcon irocRouteSeperator' style='width: 15%' >&nbsp;</td></tr>";
      }
    }
